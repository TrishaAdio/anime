...
## Bot integration (Telegram-ready captions)

Every anime object and every set page includes a `captionHtml` field: a
ready-to-send Telegram caption (`parse_mode="HTML"`). It uses `<b>` for the
title, a `<blockquote>` for the stat block, and a collapsible
`<blockquote expandable>` for the synopsis — all dynamic text is HTML-escaped so
the message can't break. No `<i>` italics are used.

```html
<b>Solo Leveling Season 2: Arise from the Shadow</b>
<blockquote>⭐ <b>Score</b> 8.52 · 8.5 IMDb
📺 TV · 13 eps · Finished Airing
📅 Winter 2025
✨ Action, Adventure, Fantasy</blockquote>
<blockquote expandable>Sung Jin-Woo, dubbed the weakest hunter of all mankind…</blockquote>
```
